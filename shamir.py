...
class Shamir:
    ...
    def generate_triples_round_1(self, n_triples):
        ...
        for i in range(self.n):
            ...
            output.append([(player_as[j], player_bs[j])
                           for j in range(len(player_as))])
        return output
    ...
    def generate_triples_round_2(self, ab_lists):
        ...
        assert 2*self.t < self.n, "triple generation requires t>n/2"
        assert len(
            ab_lists) == self.n, f"triple generation requires all {self.n} participants"
        a_shares = []
        b_shares = []
    ...
    def generate_triples_round_3(self, a_shares, b_shares, c_share_shares):
        ...
        assert 2*self.t < self.n, "triple generation requires t>n/2"
        assert len(
            c_share_shares) == self.n, "must have at least three participants"
        triples = []
...
def gen_triples(t, n, n_triples):
    ...
    for i in range(n_triples):
        ...
        for i in range(n):
            triples[i].append(TripleShare(
                a_shares[i], b_shares[i], c_shares[i]))
    return triples
