...
def run_circuit_process(t, n, c_path, index, queues, main_queue, inputs, triples):
    ...
    c = Circuit(c_path)
    outputs = c.evaluate(inputs, shamir=shamir,
                         messenger=messenger, triples=triples)
...
def test_mul32_circuit():
    ...
    processes = []
    for i in range(n):
        p = Process(target=run_circuit_process, args=(
            t, n, c_path, i+1, queues, mq, x_shares[i]+y_shares[i], triples[i]))
        processes.append(p)
    for p in processes:
        #        p.setDaemon(True
        p.start()
    ...
    for q in queues:
        ...
        q.join_thread()
    # mq.close()
    print('mq.join_thread')
    mq.join_thread()
    print("mq.join_thread TEST PASSED")
